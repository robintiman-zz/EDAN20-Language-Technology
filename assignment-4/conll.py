...
def subject_verb(formatted_corpus):
    # Find the pairs
    pairs = []
    triples = []
    freq_pairs = {}
    ...
    for sentence in formatted_corpus:
        ...
        for word in sentence:
            ...
            if deprel == 'SS':
                subject = word
                verb = sentence[int(word["head"])]
                pair = (subject["form"].lower(), verb["form"].lower())
                pairs.append(pair)
            ...
            if subject_found and object_found:
                triple = (pair[0], pair[1], object)


    # Sort the frequencies
    sorted_freq = sorted(freq_pairs.items(), key=operator.itemgetter(1), reverse=True)
